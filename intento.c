...
typedef struct nodeP {
  ...
  */
  ...
  // struct nodeP *last_member; //Puntero que apunta al 鷏timo si y s髄o si es
...
long coefGenerator(long lsup) {
  ...
  */
  ...
  /*
    ...
    leemos 驴Paso por paso?, operando de forma inmediata con el monomio
...
// Funci贸n que pide cada coeficiente del polinomio de forma ordenada por el
...
node *ingresar_plinomio(long grds) {
  ...
  for (i = grds; i >= 0;
       ...
       i--) { // Se recorre desde el grado 0 al grado m谩ximo dado "grds"
...
node *multiplicarPolinomioFBrut(node *p1, node *p2) {//Funci髇 a fuerza bruta
  ...
  while (p2 != NULL) {//Se itera en el segundo polinomio
    ...
    while (cP1 != NULL) {//Se le multiplica cada t閞mino del segundo polinomio al primer polinomio
...
// Funci贸n que elimina de la memoria la lista que contiene al polinomio

node *eliminar(node *head) {

  node *copia = NULL;

  copia = head; // Se copia para mantener el puntero del pr贸ximo nodo
...
// Funci贸n que despliega un men煤 para el usuario
...
void menu(node *head1, node *head2) {
  ...
  do {
    ...
    scanf("%d", &opcion); // Se guarda la opci髇 ingresada
    ...
    case 3:
      ...
      head1 = eliminar(head1); // Paso an谩logo al "case 1"
...
int main() {
  ...
  srand(time(NULL)); // Se inicaliza la semilla de la funci贸n srand()
